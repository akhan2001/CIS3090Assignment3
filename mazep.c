 ...
 * through a two dimensional array. This path represents a 2D maze. Added OpenMP 
 * commands to make the serial program to work in parallel with four processors.
 * @version 0.1
 * @date 2021-11-15
 * 
 ...
 * points with the thread numbers using random generated directions 
 * @param size 
 * @param x 
 * @param y 
 * @param replacement
 */
void check_neighbors(int size, int x, int y, int replacement) {
    int *randArr;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        case 1: //UP
            if (x - 2 <= 0) {
                ...
                mazeArray[x - 2][y] = replacement;
                mazeArray[x - 1][y] = replacement;
                check_neighbors(size, x - 2, y, replacement);
            }
            break;
        case 2: //RIGHT
            if (y + 2 >= size - 1) {
                ...
                mazeArray[x][y + 2] = replacement;
                mazeArray[x][y + 1] = replacement;
                check_neighbors(size, x, y + 2, replacement);
            }
            break;
        case 3: //DOWN
            if (x + 2 >= size - 1) {
...
int main(int argc, char* argv[]) {
   ...
   /*Using OpenMP fr the parallelizing of the program*/
#   pragma omp parallel num_threads(NUMTHREADS)
    {
        //Set my_rank with the thread number that starts with it
        my_rank = omp_get_thread_num();

        if (my_rank == 0) {
            //Thread 1
            mazeArray[1][1] = 0;
            check_neighbors(sizeArray, 1, 1, my_rank);
        } else if (my_rank == 1) {
            //Thread 2
            mazeArray[1][sizeArray - 2] = 1;
            check_neighbors(sizeArray, 1, sizeArray - 2, my_rank);
        } else if (my_rank == 2) {
            //Thread 3
            mazeArray[sizeArray - 2][1] = 2;
            check_neighbors(sizeArray, sizeArray - 2, 1, my_rank);
        } else if (my_rank == 3) {
            //Thread 4
            mazeArray[sizeArray - 2][sizeArray - 2] = 3;
            check_neighbors(sizeArray, sizeArray - 2, sizeArray - 2, my_rank);
        }
    }
